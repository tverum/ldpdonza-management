...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "--send-mails-coaches",
            action="store_true",
            help="Stuur mails naar de Coaches",
        )
        parser.add_argument(
            "-m",
            "--mailadressen",
...
def generate_accounts(functie):
    ...
    for lid in leden:
        ...
        ploegen = [
            Ploeg.objects.get(ploeg_id=ploeglid.ploeg.ploeg_id)
        ...
        username = "{}{}".format(
            lid.voornaam.replace(" ", "").lower(),
            lid.familienaam.replace(" ", "").lower(),
        )
        password = "{}{}".format(
            lid.voornaam[::-1].replace(" ", "").lower(),
            lid.familienaam[::-1].replace(" ", "").lower(),
        )
        try:
            user = User.objects.create_user(username, lid.email, password)
        except IntegrityError as e:
            print(
                "Account was al gecreëerd voor de persoon: {} {}".format(
